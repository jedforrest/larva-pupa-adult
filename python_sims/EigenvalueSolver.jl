...
function get_AT_CI_mixed(f, x; classical = true)
    if classical
        ...
        fwith0 = [1 + sum(x) ; f]
        NP0 = newtonPolytope(1 + sum(x), x)
        Dpol = newtonPolytope(prod(fwith0),x)
        D = getLatticePoints(Dpol)
        NPi = [newtonPolytope(prod(f[1:end .!= ii]),x) for ii = 1:length(fwith0)]
        E = [getLatticePoints(Δ) for Δ ∈ NPi]
        A₀ = getLatticePoints(NP0)
    end
    return A₀, E, D
...
# in the sense of "Toric Eigenvalue Methods for Solving Sparse Polynomial Systems".
function solve_CI_mixed(f, x; AT = nothing, complex = false, rankTol = 1e3 * eps(), verbose::Bool=false)
    if isnothing(AT)
    ...
    sol = solve_EV(f, x, A₀, E, D; complex = complex, verbose = verbose, rankTol = rankTol)
    return sol, A₀, E, D
end
